...
async def get_semantic_matches(prompt: str, threshold: float = 0.45) -> List[Dict]:
    ...
    async for cv in cursor:
        ...
        # ✅ Real-time cosine similarity-based related role generation (Top 3 only, extended_roles prioritized)
        related_roles = []
        if original_predicted_role.strip():
            try:
                ...
                full_roles = extended_roles + all_roles  # Extended roles will be checked first
